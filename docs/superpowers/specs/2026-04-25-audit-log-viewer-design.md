...
- **Authz:** owner-only. Use `require_membership(user, workspace)` with `min_role=MembershipRole.OWNER`. Rationale: the audit log can leak who did what across the team; member / viewer roles can already see the data they need through normal screens. We can relax later if a member-visible subset emerges.
